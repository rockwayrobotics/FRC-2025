...
public class ShootFromGroundDriveRotateDriveAuto extends SequentialCommandGroup {
  ...
  public ShootFromGroundDriveRotateDriveAuto(ShooterSubsystem shooter, IntakeSubsystem intake, LedSubsystem led,
      DrivebaseSubsystem drivebase, double offset, double angle) {
    m_shooter = shooter;
    m_intake = intake;
    m_led = led;
    ...
    this.addCommands(new InstantCommand(() -> System.out
        .println("Driving distance in ShootFromGroundDriveRotateFour: " + (m_drivebase.distanceDrivenAuto + offset))));

    // .then(new DriveDistance(drivebase, 0.5, () -> { return
    // m_drivebase.distanceDrivenAuto; }))

    this.addCommands(new DriveDistance(m_drivebase, 0.5, (() -> {
      return (m_drivebase.distanceDrivenAuto + offset);
    })));
