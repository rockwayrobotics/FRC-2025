...
roi = VL53L1X.VL53L1xUserRoi(6, 9, 9, 6) # run 1
# roi = VL53L1X.VL53L1xUserRoi(7, 8, 8, 7) # run 2
# roi = VL53L1X.VL53L1xUserRoi(5, 8, 10, 7) # run 3
# roi = VL53L1X.VL53L1xUserRoi(0, 8, 15, 7) # run 4
# roi = VL53L1X.VL53L1xUserRoi(7, 15, 8, 0) # run 5
...
while running:
    ...
    bar_size = int(
        (distance_in_mm / float(MAX_DISTANCE_MM)) * (cols - 10)
    )  # Scale bar_size to our terminal width
    bar = (
        BAR_CHAR * bar_size
    )  # Create a bar out of `bar_size` unicode FULL BLOCK characters
    bar = bar.ljust(
        cols - 7, " "
    )  # Pad the bar to the full with of the terminal, minus the "00.00cm " prefix
    ...
    sys.stdout.write(
        "{:04.1f}cm {}".format(distance_in_mm / 10.0, bar)
    )  # Output our measurement and bar
